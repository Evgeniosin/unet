...
usub::server::configuration::ConfigReader::ConfigReader(const std::string &config_path) {
    try {
        ...
        error_log("`ConfigReader` constructor error: %s - %s", e.what(), config_path.c_str());
        exit(9);
    }
}

std::string usub::server::configuration::ConfigReader::getKeyFilePath() {
...
std::string usub::server::configuration::ConfigReader::getPemFilePath() {
...
std::string usub::server::configuration::ConfigReader::getPort() {
...
bool usub::server::configuration::ConfigReader::is_ssl() {
    if (this->res.get_as<toml::table>("server")->contains("ssl")) {
...
std::string usub::server::configuration::ConfigReader::getIPAddr() {
...
int usub::server::configuration::ConfigReader::getIPV() {
